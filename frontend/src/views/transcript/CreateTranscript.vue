  ...
  <section>
    ...
    <div class="container-btn-term">
      <div class="group-btn ml-5">
        <button class="bg-cyan-500 hover:bg-cyan-600 font-bold py-2 px-4 rounded m-2">
          <router-link to="/admin">Home</router-link>
        </button>
        <button class="bg-cyan-500 hover:bg-cyan-600 font-bold py-2 px-4 rounded m-2">
          <router-link
            :to="`/admin/students/term/${this.$router.currentRoute.value.query.user_id}?generation_id=${this.$router.currentRoute.value.query.generation_id}`">Back</router-link>
        </button>
      </div>
    </div>
        ...
        <div class="user-transcript-info">
          ...
          <div class="term d-flex justify-between items-center mt-5">
            <div class="d-flex items-center">
              ...
              <select name="subject" v-model="selectedCourse"
                class="form-control inputstl border-1 border-zinc-500 w-52 text-black">
                <option value="" disabled>Select Subject</option>
                <option v-for="(courses, index) in courseList" :key="index" :value="courses.course">
                  {{ courses.course }}
                </option>
              </select>
            </div>
            <div class="d-flex items-center">
              ...
              <input type="number" v-model="score" class="form-control inputstl border-1 border-zinc-500 w-52" />
            </div>
            <button @click="createScore"
            ...
            <table class="min-w-full text-center">
              ...
              <tbody v-for="(score, index) in scoreSubjects" :key="index">
                <tr v-if="score.term.id == term_id && score.student.id == studentList.id">
                  ...
                  <td class="border-x border-y border-gray-500 py-2 px-4" :class="{ red: score.score < 50 }">
                    {{ score.score }}
                  </td>
                  <td class="border-x border-y border-gray-500 py-2 px-4">
                    <button type="button" data-bs-target="#editModal" data-bs-toggle="modal" @click="editScore(score.id)"
                      class="text-cyan-500 hover cursor-pointer hover:underline">
                      Edit
                    </button>
                    |
                    <button type="button" @click="score_id = score.id"
                      class="text-red-600 hover cursor-pointer hover:underline" data-bs-target="#deleteModal"
                    ...
                    <div class="modal fade" id="editModal">
                      <div class="modal-dialog">
                        <div class="modal-content">
                          ...
                          <div class="modal-body d-flex flex-col justify-around items-start h-auto">
                            <div class="d-flex items-center m-2">
                              <span class="font-bold pr-3">Course: </span>
                              <select name="subject" v-model="scoreList.course"
                            ...
                            <div class="d-flex items-center m-2">
                              <span class="font-bold pr-3">Score: </span>
                              <input type="number" v-model="scoreList.score"
                          ...
                          <div class="modal-footer d-flex justify-between">
                            ...
                            <button type="button" @click="updateScore"
                    ...
                    <div class="modal fade" id="deleteModal">
                      <div class="modal-dialog">
                        <div class="modal-content">
                          <div class="modal-header bg-cyan-500 text-white">
                            <h5 class="modal-title" id="exampleModalLabel">
                              DELETE STUDENT SCORE
                            </h5>
                          </div>
                          <div class="modal-body d-flex justify-start items-center h-20">
                          ...
                          <div class="modal-footer d-flex justify-between">
                            ...
                            <button type="button" @click="deleteScore"
...
export default {
  ...
  methods: {
    async getScoreSubject() {
      await axiosClient.get("scores").then((res) => {
