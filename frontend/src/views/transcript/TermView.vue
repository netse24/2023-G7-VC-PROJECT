<template>
  <section>
    ...
    <div class="d-flex flex-col p-10">
      ...
      <div class="w-75 d-flex m-auto border border-1 h-50 p-10 rounded shadow-[10px_35px_150px_-2px_rgba(0,0,0,0.3)]">
        <button v-for="(term, index) in termList" :key="index" @click="showTranscript(term.id)"
          class="bg-orange-500 m-2 pb-1   text-2xl font-bold rounded-lg d-flex justify-center items-center">
          <span class="bg-cyan-500 hover:bg-cyan-600 py-3 px-4 text-2xl font-bold rounded-lg flex justify-center">Term {{
            term.term }}</span>
        </button>
      </div>
    </div>
