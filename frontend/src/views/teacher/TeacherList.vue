  ...
  <section>
    ...
    <div class="w-10/12 m-auto mb-3 my-5">
      <div class="flex gap-2 my-2">
        <button class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 border-blue-700 rounded w-28">Back</button>
        ...
        <button class="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 border-blue-700 rounded w-28">
          <v-dialog v-if="selectedUsers.length>1 || selectedUsers.length==0 " class="w-50" v-model="dialogUpdate">
            <template v-slot:activator="{ props }">
              <v-text v-bind="props">Update</v-text>
            </template>
            <v-card>
              <v-card-title class="border-gray-200 bg-green-500">Update Teacher</v-card-title>
              <v-card-text>
                <v-container>
                  <p v-if="selectedUsers.length>1">Sorry you can't update more than one checkbox</p>
                  <p v-else>Please select checkbox</p>
                </v-container>
              </v-card-text>
              <v-card-actions class="flex justify-end">
                <v-btn 
                  class="bg-green" color="flex justify-center font-normal text-1xl font-bold" variant="text" @click="dialogUpdate = false">OK
                </v-btn>
              </v-card-actions>
            </v-card>
          </v-dialog>
        ...
        <button class="btn bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 border-red-800 rounded w-28">
          <v-dialog v-if="selectedUsers.length>=1 || selectedUsers.length ==0 " class="w-50" v-model="dialogDelete">
            ...
            <v-card>
              <v-card-title class="border-gray-200 bg-red-500">Delete Teacher</v-card-title>
              <v-card-text>
                <v-container>
                  <p v-if="selectedUsers.length>=1">Are you sure want to permanently delete teacher</p>
                  <p v-else>Please select checkbox</p>
                </v-container>
              </v-card-text>
              <v-card-actions class="flex justify-end">
                ...
                <v-btn v-else
                  class="bg-red" color="flex justify-center font-normal text-1xl font-bold" variant="text" @click="dialogDelete = false">OK
                </v-btn>
              </v-card-actions>
            </v-card>
          </v-dialog>
        ...
        <button class="bg-cyan-500 hover:bg-cyan-600 text-white font-bold rounded w-28">
          <v-dialog v-if="selectedUsers.length>1 || selectedUsers.length==0 " class="w-50" v-model="dialogDetail">
            <template v-slot:activator="{ props }">
              <v-text v-bind="props">See more</v-text>
            </template>
            <v-card>
              <v-card-title class="border-gray-200 bg-cyan-500">Detail Teacher</v-card-title>
              <v-card-text>
                <v-container>
                  <p v-if="selectedUsers.length>1">Sorry you can detail teacher one by one</p>
                  <p v-else>Please select checkbox</p>
                </v-container>
              </v-card-text>
              <v-card-actions class="flex justify-end">
                <v-btn 
                  class="bg-cyan"
                  color="flex justify-center font-normal text-1xl font-bold" variant="text" @click="dialogDetail = false">OK
                </v-btn>
              </v-card-actions>
            </v-card>
          </v-dialog>
          <div v-else> 
            <router-link :to="`/admin/batch/teacher_detail/${selectedUsers}`" >See more</router-link> 
          </div>
        </button>
      </div>
        ...
        <tbody>
          <tr v-for="(teacher, index) in teachers" :key="index">
            <td class=" border border-slate-300 pl-4">
              <input type="checkbox" id="checkbox" v-model="selectedUsers" :value="teacher.id" class="accent-cyan-500 w-4 h-4 rounded"/>
            </td>
            <td class="py-2 px-4  border border-slate-300">{{teacher.user.first_name}}</td>
            <td class="py-2 px-4  border border-slate-300">{{teacher.user.last_name}}</td>
            <td class="py-2 px-4  border border-slate-300">{{teacher.course_id.course}}</td>
          </tr>
        </tbody>
      </table>
    </div>
