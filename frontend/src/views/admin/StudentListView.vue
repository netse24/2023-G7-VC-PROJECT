  ...
  <section>
    ...
    <div class="w-11/12 m-auto">
        ...
        <div class="flex gap-2 admin-page" v-if="user_role != null && user_role == 'admin'">
          ...
          <button class="bg-red-600 hover cursor-pointer hover:bg-red-700  font-bold py-2 px-4 rounded"
            ...
            <v-dialog class="w-5/12" v-model="dialogDelete" v-if="selectedUsers.length >= 1">
              ...
              <v-card>
                <v-card-title class="border-gray-200 bg-cyan-500"> DELETE DATA OF STUDENT </v-card-title>
                <v-card-text>
                ...
                <v-card-actions class="d-flex justify-end gap-5">
                  <div v-if="selectedUsers.length >= 1">
                    ...
                    <v-btn v-if="selectedUsers.length > 0" @click="deleteStudent((dialogDelete = false))"
                      class="bg-red text-white w-20" color="font-normal text-1xl font-bold">Yes
                    </v-btn>
                  </div>
            ...
            <v-dialog class="w-50" v-if="selectedUsers.length == 1" v-model="dialogUpdate">
              ...
              <v-sheet width="auto" class="pa-5 pb-7" v-if="model != null">
                <v-form @submit.prevent="updateStudent">
                  <v-row class="d-flex">
                    <v-col>
                      <v-text-field required density="compact" v-model="model.first_name" label="Enter Firstname"
                        :value="model.first_name" :rules="firstNameRules" prepend-inner-icon="mdi-account-box">
                      </v-text-field>
                    </v-col>
                    <v-col>
                      <v-text-field density="compact" v-model="model.last_name" label="Enter Lastname"
                        :rules="lastNameRules" prepend-inner-icon="mdi-account-box">
                      </v-text-field>
                    </v-col>
                  </v-row>
                  <v-row>
      ...
      <div class="d-flex mt-5 justify-between">
        <div>
          <button class="bg-cyan-500 hover:bg-cyan-600  font-bold py-2 px-4 border-blue-700 rounded w-auto mr-2"
            @click="showPreviousClasses">
            Previous
          </button>
        </div>
        <div class="w-full">
          <button v-for="className in displayedClasses" :key="className" @click="selectedClass = className"
